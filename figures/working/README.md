...
| `fig6_valve_units_lag` | Line 7e's reading vs the true share of net as child care rises; 7e reaches 40% at $590/wk when the true burden is 55%; the true burden passes 40% at $130/wk | `fig6_valve.py`; `submission_figures.py` |
...
**Exhibit set (one chart per file, `E01`–`E27`, 23 files, no exhibit with more than one set of axes): `output/charts/exhibits/`, built by `exhibits.py` from the CSVs above and copies of the single-panel figures; naming and ratings in `docs/2026-09-05-chart-exhibit-guide.md`.** Measured on the 3-child Box 1 grid (1,147 cells), after the 2026-09-08 fix to net_position.py's treatment of who claims the children for tax purposes at equal parenting time: recipient household ahead in 15% (76% below $150k higher-earner income, none above $175k); payor ahead per person in 99% (unchanged); order above 40% of payor net in 11%, all at lower-earner income of $20k or less.
