...
// $33/wk (lower earner) / $43/wk (higher earner), matching the convention already used by
// model/charts/_common.py's `order()` for every heatmap in this project (lower earner always
// gets the $33 premium, higher earner the $43 premium, regardless of which one the worksheet
// ends up naming "payor"). The MA Child and Family Tax Credit for children under 13 is fixed at
// ZERO qualifying children on this tool -- the same generic-grid convention as every heatmap on
// this site (there is no control for how many of the children are under 13) -- so the
// recipient-household figures below run slightly LOWER than the site's own
//
// v3 (2026-09-07) adds a third control, child care, as a discrete scenario choice rather than a
// fourth slider -- a slider's continuous range does not match how this site's own findings
// state the number (a fixed $300/wk claim, model/submission_figures.py SS2.1), and a fourth
// slider on top of two income sliders and two segmented controls would break the tool's word/
// control budget. Three options: "None" (the v2 default, unchanged), "Recipient pays" (Parent A
// -- the lower earner in this tool's convention -- pays $300/wk to the provider out of pocket,
// reimbursed in part through Line 6b exactly as the worksheet already does), and "Both pay"
// (each parent pays their own $300/wk, the ordinary case at equal parenting time, $600/wk
// combined -- model/submission_figures.py SS2.1's "both pay" scenario). Composition into
// net-position.js's analyze(): weeklyChildcare is the two parents' own amounts summed;
// payorChildcareShare is whichever parent the worksheet names payor, their own amount, divided
// by the total. This is the SAME analyze() call v2 already made (payorChildcareShare=0), not a
// new function -- see computeWithFacts() below. The "true share of the payor's net income" cell
// now reads burden_pct_of_payor_net instead of support_pct_of_payor_net (identical whenever the
// payor pays no child care directly, so v2's numbers are unchanged; it only differs in the
// "both pay" scenario, where the payor's own $300/wk must count toward their true burden).
// Correctness gate: assets/js/fixtures/calculator-childcare.json, generated by
// tools/gen_calculator_childcare_fixtures.py from this repo's own model/, checked by
// assets/js/calculator.test.js PART 6 against every one of 108 combinations.
...
//   example's real incomes, the three `data-calc-radio` groups MUST default to kids=3/box=1/
//   childcare=0 (the `checked` attribute), and every `data-calc-cell` span MUST already contain
//   that baseline's real, precomputed numbers as static text ($1,013/wk, 26.5%, 37.7%, $87,172/yr,
//   $92,941/yr, $23,235/yr -- see assets/js/fixtures/calculator-v2.json,
//   kids=3/box=1/higher=201000/lower=29640, and assets/js/fixtures/calculator-childcare.json's
//   matching childcare=0 row, which reproduces the identical numbers).
...
//
// FIXED FACTS, not sliders (per design brief SS3.5's word budget): health premiums $33/wk lower
// earner, $43/wk higher earner, MA under-13 credit fixed at zero qualifying children. Children,
// custody and (as of v3) child care are all real controls (see MCSGCalculatorFacts).
...
// Fixed weekly amount per child-care scenario -- see the v3 header comment above for why this is
// a discrete scenario choice (childcare: 0/1/2) rather than a fourth slider.
var CHILDCARE_AMOUNT = 300.0;
...
(function () {
  ...
  // childcare scenario (0/1/2) -> [Parent A's own weekly amounts, Parent B's own weekly amounts],
  // each an array of per-child amounts as worksheet.js's run() expects. Parent A is always the
  // lower earner in this tool's convention (see below) -- "recipient pays" assigns the cost to
  // Parent A because that is the worksheet's usual recipient; "both pay" is symmetric so which
  // parent ends up payor does not matter to the composition below.
  function childcareArraysFor(scenario) {
    if (scenario === 1) return { a: [CHILDCARE_AMOUNT], b: [] };
    if (scenario === 2) return { a: [CHILDCARE_AMOUNT], b: [CHILDCARE_AMOUNT] };
    return { a: [], b: [] };
  }
  ...
  function computeWithFacts(higherAnnual, lowerAnnual, facts) {
    ...
    // Combine each parent's own out-of-pocket child care into the single weekly figure and
    // payor-share fraction net-position.js's analyze() already takes -- the SAME call v2 made
    // (payorChildcareShare=0), not a new function. weeklyChildcare is the two parents' own
    // amounts summed; payorChildcareShare is the worksheet's payor's own amount over that total.
    // Reduces to 0 whenever only Parent A pays and Parent B is payor (the "recipient pays"
    // scenario's usual case), and to exactly 0.5 in "both pay" regardless of which parent the
    // worksheet names payor, since both contribute the same amount -- see
    // tools/gen_calculator_childcare_fixtures.py's header comment for the same composition,
    // verified there against model/submission_figures.py SS2.1's manual arithmetic.
    ...
    var pos = N.analyze(payorGross, recipGross, facts.kids, r['7d'], weeklyChildcare, payorChildcareShare, undefined, 0);
    return {
      order_wk: r['7d'],
      line_7e: r['7e'],
      // burden_pct_of_payor_net, not support_pct_of_payor_net -- the two are identical whenever
      // the payor pays no child care directly (v2's only case), but the "both pay" scenario needs
      // the payor's own $300/wk counted toward their true burden, which support_pct_of_payor_net
      // never does. See the v3 header comment above.
