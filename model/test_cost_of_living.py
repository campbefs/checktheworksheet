...
f = c.facts()
ma, up = c.costlier()
check("Massachusetts price level is 105.8", round(f["ma_rpp"], 1) == 105.8)
...
check("every row has a price level and both orders", all(r["rpp"] and r["equal"] and r["primary"] for r in c.rows()))
check("forty-nine or fifty jurisdictions joined (Georgia held out)", len(c.rows()) >= 49)
# The price levels the page quotes, rounded as printed.
for st, v in [("California", 110.7), ("Hawaii", 110.0), ("District of Columbia", 109.9), ("New Jersey", 108.8),
              ...
              ("Connecticut", 103.6)]:
    check(f"{st} price level is {v}", round(c.state(st, "rpp"), 1) == v)
