...
def main():
    ...
    theme.finish(ax, title=f"Massachusetts's equal-time order beats {below} of {len(others)} others' primary-custody orders",
                 subtitle="Monthly order: Massachusetts under Box 1 (children half the time each) against every other jurisdiction with the children primarily with the lower earner.",
                 comma=False,
